...
from convlab2.policy.mle.crosswoz import MLE as MLEPolicy
...
def test_end2end():
    ...
    user_nlu = BERTNLU(mode='sys', config_file='crosswoz_all.json',
                       model_file='https://convlab.blob.core.windows.net/convlab-2/bert_crosswoz_all.zip')
    # not use dst
    user_dst = None
    # rule policy
    user_policy = MLEPolicy()
    # template NLG
    user_nlg = TemplateNLG(is_user=True)
    # assemble
    user_agent = PipelineAgent(user_nlu, user_dst, user_policy, user_nlg, name='user')

    analyzer = Analyzer(user_agent=user_agent, dataset='crosswoz')

    set_seed(20200202)
    analyzer.comprehensive_analyze(sys_agent=sys_agent, model_name='BERTNLU-RuleDST-MLEPolicy-TemplateNLG_cro', total_dialog=1000)
