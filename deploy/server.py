...
class ServerCtrl(object):
    ...
    def on_close(self, token):
        if not self.sessions.has_token(token):
            raise DeployError('deploy.server.ServerCtrl#on_close No such token:\'%s\'' % token)
    ...
    def on_clear_expire(self):
        ...
        for (token, session) in expire_session.items():
            print("deploy.server.ServerCtrl#on_clear_expire token:",token)
    ...
    def on_response(self, token, input_module, data, modified_output={}):
        if not self.sessions.has_token(token):
            raise DeployError('deploy.server.ServerCtrl#on_response No such token:\'%s\'' % token)
    ...
    def on_modify_last(self, token, modified_output):
        if not self.sessions.has_token(token):
            raise DeployError('deploy.server.ServerCtrl#on_modify_last No such token:\'%s\'' % token)
        session = self.sessions.get_session(token)
    ...
    def on_rollback(self, token, back_turns=1):
        if not self.sessions.has_token(token):
            raise DeployError('deploy.server.ServerCtrl#on_rollback No such token:\'%s\'' % token)
        session = self.sessions.get_session(token)
    ...
    def _turn(self, last_turn, model_map, history, input_module, data, modified_output):
        ...
        for mod in MODULES:
            ...
            if temp_data is not None and model_map[mod] is not None:
                # on response
                param = [temp_data]
                print("deploy.server.ServerCtrl#_turn mod:{0},dst_model:{1}".format(mod,dst_model))
                if mod == 'nlu' and dst_model is not None:
                    context = [x[1] for x in dst_model.state['history'][:-1]]
                    param.append(context)

                # if mod == 'dst':
                #     dst_model.state['user_action'] = temp_data
                print("deploy.server.ServerCtrl#_turn param:", param)
